    ...
    if(summarizationApiAvailable) {
      ...
      const returnData= {
        ...
        key_points: summary.split('\n')
            .map((point: string) => point.trim().replace(/^\*\s*/, '').replace(/\*\*([^*]+)\*\*/g, '<b>$1</b>'))
      ...
      <main className="flex flex-1 flex-col gap-8 pt-20 w-[70%]">
        {!isValidBrowser ?
          <>
            ...
            <p className='text-center text-neutral-600'>You should use chrome (&gt;=129.0.6639.0) for offline mode.</p>
          </> : !summarizationApiAvailable ?
          <>
            <h1 className="text-4xl font-bold text-center">API not available</h1>
            <p className='text-center text-neutral-600'>Try upgrading your chrome (&gt;=129.0.6639.0) or check requirements.</p>
          </> :
          <>
